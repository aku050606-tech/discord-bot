...
class HUWaitView(discord.ui.View):
    ...
    @discord.ui.button(label="◀️ 中止（参加費返却）", style=discord.ButtonStyle.secondary)
    async def cancel(self, interaction, button):
        room = poker_rooms.get(self.room_id)
        if not room or room.get("phase") != "waiting":
            await interaction.response.send_message("いまは中止できません", ephemeral=True)
            return
        if str(interaction.user.id) != room["players"][0]["id"]:
            await interaction.response.send_message("募集者だけが中止できます", ephemeral=True)
            return
        # 募集者のアンティを返却して募集を閉じ、カジノへ戻る
        db.update_balance(room["players"][0]["id"], room["guild_id"], room["ante"])
