...
class SkillDetailSelect(discord.ui.Select):
    ...
    async def callback(self, it):
        ...
        lines = [f"種別：{sk.get('type','—')}", f"ヒット数：{sk.get('hits',1)}"]
...
def build_skill_zukan_embed(uid=None):
    ...
    for sid, s in sorted(VS.SKILLS.items(), key=lambda x: (x[1].get("rank", 2), x[0])):
        ...
        if s.get("slot") == "weapon":
            ...
            tags = "胴・脚"
        meta = []
        if s.get('cooldown'): meta.append(f"CD{s['cooldown']}")
        if s.get('charge'): meta.append(f"溜め{s['charge']}")
        if s.get('hits', 1) and s.get('hits', 1) > 1: meta.append(f"{s['hits']}HIT")
        suffix = ('／' + '・'.join(meta)) if meta else ''
        lines.append(f"{stars} {s['emoji']} **{s['name']}**　[{tags}]{suffix}")
    emb.description = "\n".join(lines)
    return emb
