...
md.append('- E1海戦コイン補正: `0.75`（E1だけ稼ぎ過多を抑える。E2以降は1.0）\n')
md.append('- 黄金の羅針盤: 海探索の船倉コイン系報酬にも反映。イベント/発見/敵撃破の報酬倍率に `×1.6`。\n')
...
md.append('\n## 無人島：上陸後の内訳\n\n')
md.append('無人島そのものの出現率は `AREA_ENCOUNTERS` の `island` 重みで管理します。下記は「上陸する」を押した後の結果内訳です。\n\n')
md.append('|結果|割合|内容|\n|---|---:|---|\n')
md.append(f'|宝|{V.ISLAND_TREASURE_RATE * 100:.1f}%|船倉コイン `{V.ISLAND_TREASURE["base_min"]:,}〜{V.ISLAND_TREASURE["base_max"]:,}` + 発見EXP/ドロップ|\n')
md.append(f'|何もなし|{V.ISLAND_EMPTY_RATE * 100:.1f}%|報酬なし・ペナルティなし|\n')
md.append(f'|伏兵|{V.ISLAND_AMBUSH_RATE * 100:.1f}%|その海域の敵プールから伏兵戦闘|\n')
md.append(f'|罠/ペナルティ|{V.ISLAND_PENALTY_RATE * 100:.1f}%|HP約{V.ISLAND_PENALTY_HP_PCT * 100:.0f}%減・燃料約{V.ISLAND_PENALTY_FUEL_PCT * 100:.0f}%減|\n')
