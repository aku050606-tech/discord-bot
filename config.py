...
#
#   ※ モンテカルロ検算済み（パッチ1.02：機械割を引き下げ＋ハイパービッグ追加）:
#       設定1:98.2% / 2:99.7% / 3:101.4% / 4:104.0% / 5:107.5% / 6:113.3%
#       子役のみ戻り 44.3%（全設定共通） / ハイパービッグ寄与 +3.0%（全設定共通）
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
JUGGLER_KOYAKU = [
    ...
    ("budou",   1/5.9, 96,  "ぶどう",   ("🍇", "🍇", "🍇")),
    ("cherry",  1/35,  40,  "チェリー", ("🍒", "　", "　")),
    ("pierrot", 1/430, 200, "ピエロ",   ("🃏", "🃏", "🃏")),
    ("bell",    1/430, 200, "ベル",     ("🔔", "🔔", "🔔")),
]

# ── ボーナス純増（ナトコイン）。BIG:REG ≒ 2.5:1（実機比準拠）──
JUGGLER_BIG_NET = 7000
JUGGLER_REG_NET = 2800
JUGGLER_HYPER_NET = 15000   # ★ハイパービッグ（プレミアフラグ）。通常BIGの約2倍

# ── ハイパービッグ出現率（全設定共通の固定プレミア。1/8192）──
#   設定差は付けない＝どの台でも夢として等しく降ってくるプレミアフラグ。
JUGGLER_HYPER_RATE = 1/8192

# ── 設定別ボーナス確率（1/n）。機械割の本体。設定6だけBIG=REG（実機の味）──
#   ハイパービッグは全設定共通(JUGGLER_HYPER_RATE)。設定差はBIG/REGのみで付ける。
JUGGLER_BONUS = {
    1: {"big": 1/291, "reg": 1/439, "hyper": JUGGLER_HYPER_RATE},
    2: {"big": 1/289, "reg": 1/398, "hyper": JUGGLER_HYPER_RATE},
    3: {"big": 1/283, "reg": 1/359, "hyper": JUGGLER_HYPER_RATE},
    4: {"big": 1/277, "reg": 1/322, "hyper": JUGGLER_HYPER_RATE},
    5: {"big": 1/266, "reg": 1/286, "hyper": JUGGLER_HYPER_RATE},
    6: {"big": 1/249, "reg": 1/248, "hyper": JUGGLER_HYPER_RATE},
}
