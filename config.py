...
PORT_DAILY_FUND_AMOUNT = 2_000_000
...
#   設定1≈100% / 設定6≈124%（継続まわりは据え置き。出玉感はGRAVITY BURSTで付与）。
...
# ── ⚡ GRAVITY BURST（GOD中の大型上乗せイベント）──
#   AT中の各ゲームで GOD_BURST_RATE の確率で発動。通常の上乗せに加えて
#   一気にドカンと乗る"飛び道具"（釣りの宝箱/ボスに相当）。専用演出でバラす。
#   ※ ルート/ランク/設定に依存しない共通イベント（全員に等しく訪れるご褒美）。
GOD_BURST_RATE = 1/65
GOD_BURST_TABLE = [   # (上乗せ額, 重み)
    (1200,  0.60),
    (2500,  0.28),
    (5000,  0.10),
    (10000, 0.02),    # 特異点級（激レア）
]
# 額に応じた演出（しきい値, タイトル, サブ, RGB）※上から判定し最初に超えたもの
GOD_BURST_EFFECTS = [
    (10000, "🌌 SINGULAR BURST", "―― 特異点級の、暴走。", (255, 215, 0)),
    (5000,  "🌋 GRAVITY OVERLOAD！！！", "重力が、決壊した──", (255, 90, 0)),
    (2500,  "💥💥 重力崩壊！！", "歪みが、一気に弾けた。", (255, 60, 120)),
    (0,     "💥 GRAVITY BURST！", "ぐにゃりと、空間が裂ける。", (180, 0, 220)),
]
...
SLOT_WAIT = {
    ...
    "rankup":   1.2,   # 昇格演出
    "feint":    1.4,   # フェイントのタメ
    # ⚡ GRAVITY BURST 開示タメ（額が大きいほど長く溜める）
    "burst":       1.4,
    "burst_big":   2.2,   # 5000以上
    "burst_max":   3.0,   # 10000（特異点級）
}
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 🎖️ 設定推測メダル（GRAVITAS GAME 終了画面の示唆）
#   ・一撃終了ごとに抽選。出たメダルは"必ず本物"（設定が条件を満たす時しか出ない）。
#   ・銅=設定2以上 / 銀=4以上 / 金=5以上 / NATO=6確定。確定系ほど控えめ。
#   ・絶対に嘘をつかない＝銅は設定1では出ない、NATOは設定6でしか出ない、等。
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 表示情報: key -> (emoji, 名称, 示唆文, RGB)
#   ※ NATO章は専用カスタム絵文字に差し替え可（例 "<:nato:1234567890>"）。
GOD_BADGE_INFO = {
    "bronze": ("🥉", "銅メダル", "設定② 以上濃厚", (205, 127, 50)),
    "silver": ("🥈", "銀メダル", "設定④ 以上確定", (192, 192, 192)),
    "gold":   ("🥇", "金メダル", "設定⑤ 以上確定", (255, 200, 0)),
    "nato":   ("🔵", "NATO章",  "設定⑥ 確定！！", (0, 120, 255)),
}
# 設定 -> [(badge_key or None, 確率)]  ※合計1.0、None=示唆なし。上から累積で判定。
GOD_SETTEI_BADGES = {
    1: [(None, 1.00)],
    2: [("bronze", 0.12), (None, 0.88)],
    3: [("bronze", 0.14), (None, 0.86)],
    4: [("bronze", 0.11), ("silver", 0.07), (None, 0.82)],
    5: [("bronze", 0.11), ("silver", 0.07), ("gold", 0.05), (None, 0.77)],
    6: [("bronze", 0.10), ("silver", 0.08), ("gold", 0.06), ("nato", 0.04), (None, 0.72)],
}
